...
import { BRANCH_TO_ZONES } from '@/data/mockData';
...
export default function Dashboard() {
      ...
      .then(({ data, error }: { data: any; error: any }) => {
        ...
        if (data) {
          ...
          if (user.role === 'HS-ADMIN') {
            ...
            availableBranches = (uniqueBranches as string[]).filter((b: string) => userBranches.includes(b));
          }
  ...
  useEffect(() => {
    if (selectedBranch && BRANCH_TO_ZONES[selectedBranch]) {
      const branchZones = BRANCH_TO_ZONES[selectedBranch];
      setZones(branchZones);
      // Set to first zone (which could be "SHOP CLOSED" or "ZONE 1")
