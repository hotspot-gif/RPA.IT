...
async function addChartToPDF(pdf: jsPDF, cid: string, x: number, yp: number, cw: number, ch: number, title: string) {
  ...
  if (!el) {
    console.warn('PDF export: Element not found', cid);
    return;
  }
  try {
    ...
    const canvas = await html2canvas(el, { 
      scale: 1.5, // Slightly lower scale for performance
      useCORS: true, 
    ...
    const url = canvas.toDataURL('image/png', 0.90);
    ...
    console.error('PDF export: Chart fail:', cid, e);
  }
}
...
export async function generatePDF(summary: RetailerSummary, monthly: RetailerMonthly[], user: any, setProgress?: (p: number) => void) {
  try {
    ...
    setProgress?.(20);
    console.log('PDF: Capturing Cover Charts');
    ...
    y += 0.5;
    console.log('PDF: Capturing Plan Charts');
    ...
    ['2024', '2025', '2026'].forEach((y2, ri) => {
      const d = yd(y2);
      const pv = ri > 0 ? yd(['2024', '2025', '2026'][ri - 1]) : null;
    ...
    y += 0.5;
    console.log('PDF: Capturing Port-In Charts');
    ...
    // Page 3 — Monthly data
    pn++; setProgress?.(60);
    console.log('PDF: Page 3 Monthly');
    ...
    pn++; setProgress?.(95);
    console.log('PDF: Page 5 Deductions');
    ...
    y += 4;
    console.log('PDF: Capturing Final Charts');
