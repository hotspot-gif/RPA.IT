...
export async function generatePDF(summary: RetailerSummary, monthly: RetailerMonthly[], user: any, setProgress?: (p: number) => void) {
  ...
  doc.text('Retailer Performance Report', 50, y + 5);
  // Add logo placeholder
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text(`Generated by: ${user.full_name} (${user.email})`, 14, y + 14);
  doc.text(`Branch: ${summary.branch} | Zone: ${summary.zone}`, 14, y + 22);
  doc.text(`Retailer: ${summary.retailer_id}`, 14, y + 30);
  doc.text(`Generated: ${new Date().toLocaleDateString('en-IE', { day: '2-digit', month: 'long', year: 'numeric' })}`, 14, y + 38);
  y = 60;
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['Port-In Total', fmtN(summary.port_in)],
      ['Port-Out', fmtN(summary.port_out)],
      ['Port in Incentive', fmtN(summary.pi_raw)],
      ['Gara bonus', fmtN(summary.add_gara)],
      ['Total Port in bonus', fmtN(summary.pi_total)],
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['Port-In Total', ...yearlyTotals.map(yt => fmtN(yt.port_in))],
      ...
      ['Port-Out', ...yearlyTotals.map(yt => fmtN(yt.port_out))],
      ['Port in Incentive', ...yearlyTotals.map(yt => fmtN(yt.pi_raw))],
      ['Gara bonus', ...yearlyTotals.map(yt => fmtN(yt.add_gara))],
      ['Total Port in bonus', ...yearlyTotals.map(yt => fmtN(yt.pi_total))],
  ...
  autoTable(doc, {
    startY: y,
    head: [['Month', 'Incentive', 'GA', 'Port-In Total', 'PI≤6.99', 'PI>6.99', 'NP≤6.99', 'NP>6.99', 'Port in Incentive', 'Gara bonus', 'Total Port in bonus', 'Deductions', 'Renewal%']],
    body: sortedMonthly.map(m => {
      const [yr, mo] = m.month.split('-');
      const label = `${MONTH_NAMES[parseInt(mo) - 1]} ${yr}`;
      return [label, fmt(m.incentive), fmtN(m.ga_cnt), fmtN(m.port_in), fmtN(m.pi_l6), fmtN(m.pi_g6), fmtN(m.np_l6), fmtN(m.np_g6), fmtN(m.pi_raw), fmtN(m.add_gara), fmtN(m.pi_total), fmt(m.total_ded), fmtP(m.renewal_rate)];
    }),
    theme: 'striped',
  ...
  autoTable(doc, {
    ...
    body: sortedMonthly.map(m => {
      const [yr, mo] = m.month.split('-');
      const label = `${MONTH_NAMES[parseInt(mo) - 1]} ${yr}`;
      return [label, fmtN(m.pi_raw), fmtN(m.add_gara), fmtN(m.pi_total), fmtN(m.port_in), fmtN(m.port_out)];
    }),
    theme: 'striped',
    headStyles: { fillColor: [33, 38, 78], textColor: 255, fontSize: 8 },
    bodyStyles: { textColor: [33, 38, 78], fontSize: 8 },
